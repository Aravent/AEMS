...
{	 
 	...
 	while(1)
	{
		...
		{					   
			...
			for(ss = 0;ss <= len;ss++)
			{
				send_data[ss] = USART_RX_BUF[ss];
			}
			
//			for(dd = 0;dd <= len;dd++)
//			{
//				send_nymph_data[dd] = send_data[dd];
//			}
			...
			for(h=0;h<s;h++)
			{
				for(i=0;i<8;i++)
				{
					canbuf[i]=send_nymph_data[r];//填充发送缓冲区
					USART_SendData(USART1,send_nymph_data[r]);//向串口1发送数据
